        ...
        final StringRequest loginRequest = new StringRequest(Request.Method.POST, url + "login", new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                   //TODO should parse the json response to redirect to appropriate functions.
                    Log.d("login.FOUND_RESPONSE", response);
                    if (response.contains("success")) {
                        //initialize the activity(page)/destination
                        //Intent listPage = new Intent(Login.this, ListViewActivity.class);
                        Intent listPage = new Intent(Login.this, MainActivity.class);
                        ...
                        if (response.contains("exist"))
                            message_toast = "No user with that email exists.";
                        else
                            message_toast = "Incorrect password";
                        //Toast.makeText(getApplicationContext(), message_toast, Toast.LENGTH_SHORT).show();
                        message.setText(message_toast);
                    }
                }
            ...
            protected Map<String, String> getParams() {
                ...
                params.put("android", "hooray for less security by removing reCAPTCHA...!");
